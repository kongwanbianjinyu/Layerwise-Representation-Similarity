...
def inference_one_instance(args, data, model, index, k, alpha, cuda_available, device):
    ...
    for idx in range(num_per_instance):
        if args.decode_method == 'greedy':
            output = model.greedy_search(input_tensor, args.decoding_len)
        elif args.decode_method == 'beam':
            output = model.beam_search(input_tensor,k, args.decoding_len)
        elif args.decode_method == 'sampling':
            output = model.nucleus_sampling(input_tensor,args.p, args.decoding_len)
        elif args.decode_method == 'contrastive':
...
def parse_config():
    ...
    parser.add_argument("--exit_layer", type=int, default=11, help="the layer to exit")
    parser.add_argument("--decode_method", type=str, choices=['greedy', 'beam', 'sampling', 'contrastive'], default='greedy')
...
if __name__ == '__main__':
    ...
    from simctg import EarlyExitSimCTG
    model = EarlyExitSimCTG(args,50256) #50256, data.pad_token_id
    if cuda_available:
